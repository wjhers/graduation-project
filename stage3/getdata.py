...
def figureHeatmap(mydata,filename,xtick=2,ytick=2,xx='X-Edges Index'): # mydata二维矩阵,heatmap图
    ...
    ax.set_ylabel('Y-Edges Index',fontsize=10)
...
def figureFilesTimes(mydata1,mydata2,mydata3,c1="File-0",c2="File-1",c3="File-2"):
    fig = plt.figure()
    markes = ['-o','-s','-^']
...
def hourRequests():
    ...
    figureHeatmap(Edge_T,"Edge_T",xtick=50,ytick=20,xx="X-Hours")


# 绘制智能体的奖励值
def figureRewarld(LossSet,ag_name,xx="X-Iters",yy="Y-Rewarld"):
    plt.plot([x for x in range(len(LossSet))],LossSet)
    plt.xlabel(xx)
    plt.ylabel(yy)
    plt.title(ag_name+' rewarlds') 
    plt.legend()
    plt.savefig("./results/agents/"+ag_name+'_rewarlds.png')
    
    print("./results/agents/"+ag_name+'_rewarlds.png---|saved!')
    # plt.show()
    plt.close()
...
if __name__ == "__main__":
    ...
    sumRequests(begin=24,end=48,day="Tuesday")
