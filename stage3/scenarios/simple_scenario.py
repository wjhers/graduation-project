...
class Scenario(BaseScenario):
    ...
    def mincost(self,agent,world,f_id,action):
        ...
        for n_id in neighbors:
            ...
            for ii in n_id_lochit:
                if ii == f_id:
                    isnehit = True
                    # cost = int(param.alpha*param.ne_latency(agent.ag_id,n_id)+param.beta*param.traffic_cost_neighbor)
    ...
    # agent每个没命中的文件需要的最小开销
    # def mincost(self,agent,world,f_id):
        # # 得到agent的所有邻居 ndarray
        # neighbors = csr.Neighbors(agent.ag_id)
        # # 提供最小开销的近邻/cdn
        # # 几个近邻都一样，选择id最小的近邻
        ...
        # cost1 = int(alpha*d_f_e_t*l_e_j_t + beta*d_f_e_t*p_e_j_t + beta*p_e_j_t)
        # return 0-cost1
